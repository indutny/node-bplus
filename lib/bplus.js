...
BPlus.prototype.getRange = function getRange(start, end) {
  var promise = new process.EventEmitter;

  if (typeof start === 'string') start = new Buffer(start);
  if (typeof end === 'string') end = new Buffer(end);

  this._db.getRange(start, end, function(err, type, key, value) {
    if (err) return promise.emit('error', err, type);

    if (type === 'message') {
      promise.emit('message', key.value, value.value, key.ref, value.ref);
    } else {
      promise.emit('end');
    }
  });
